        ...
        for idx, element in enumerate(new_tbl[i][19:]):#tag之後的資料
            if element[0:3] == '***':
                ...
                if idx in [0]:
                    ...
                    style_other_date.num_format_str = fmt
                    sheet1.write(i+1, len(new_tbl[i][:19])+4+idx, style=style_other_date)                  
                else:
...
def writeListFile(mylist):
    ...
    header = ['項次', '申請人或機構', '案件型別', '同意備案\n核准容量(kW)', '設置位置', '縣市', '設置場址(地址)', '設置場址(地號)', '統計分類', '售電方式', '同意備案\n申請日期', '同意備案\n核准日期', '案件狀態', '聯絡人姓名', '聯絡人電話', '備案編號', '簽約日期', '完工併聯\n日期', '完工併聯\n容量(kW)', 'TPC', '台糖', '大業者', '工業局\n工業區','總土地面積\n(平方公尺)','使用分區','用地類別','併網審查\n受理編號','階段','是否管考','能源統計\n月報計入','電訪人員', '關鍵事項', '主責單位', '預計完成時間', '辦理情形','案件預計完工時間','案件預計階段','案件最新階段完成時間','電訪日期','案場施工狀況','案場問題分類 ','案件問題描述','案場預計完工日','台電問題分類','台電問題描述','預計併聯日期','備註','工作許可名稱','設置容量(kW)','電源線引接點','申請施工許可日期','取得施工許可日期','施工許可辦理情形說明']
    # mylist =  [['羅雅蓮', '第三型', '4.8', '屋頂型', '新竹縣', '新竹縣芎林鄉綠獅一街96號', '新竹縣芎林鄉綠獅段1069-0011地號', '住宅建物屋頂', '售電', '2019-7-29', '2019-8-19', '已有併聯紀錄', '鍾俊業', '03-5352-280', '108PV1123', '2019-9-16', '2019-11-20', '4.8', [True, False, True, False], '關鍵事項', '主責單位', '預計完成時間', '辦理情形'], ['勝陽能源股份有限公司', '第三型', '498.96', '屋頂型', '台北市', '台北市士林區承德路4段177號', '台北市士林區光華段四小段0793-0000地號', '地方公有屋頂', '售電', '2019-8-13', '2019-8-19', '已有併聯紀錄', '太陽能系統處/黃子嘉', '02-25984299#670', '108PV1124', '', '2019-12-9', '498.96',[False, False, True, False], '關鍵事項', '主責單位', '預計完成時間', '辦理情形']]
    #print("mylistqqqq",mylist[0])
    #for i in range(len(mylist)):
    #    for j in range(len(mylist[i])):
...
def writeListFile2(mylist):
    print("start write list file")    
    header = ['項次', '申請人或機構', '案件型別', '同意備案\n核准容量(kW)', '設置位置', '縣市', '設置場址(地址)', '設置場址(地號)', '統計分類', '售電方式', '同意備案\n申請日期', '同意備案\n核准日期', '案件狀態', '聯絡人姓名', '聯絡人電話', '備案編號', '簽約日期', '完工併聯\n日期', '完工併聯\n容量(kW)', 'TPC', '台糖', '大業者', '工業局\n工業區','總土地面積\n(平方公尺)','使用分區','用地類別','併網審查\n受理編號','階段','是否管考','能源統計\n月報計入','電訪人員','電訪日期','案場施工狀況','案場問題分類 ','案件問題描述','案場預計完工日','台電問題分類','台電問題描述','預計併聯日期','備註']
    # mylist =  [['羅雅蓮', '第三型', '4.8', '屋頂型', '新竹縣', '新竹縣芎林鄉綠獅一街96號', '新竹縣芎林鄉綠獅段1069-0011地號', '住宅建物屋頂', '售電', '2019-7-29', '2019-8-19', '已有併聯紀錄', '鍾俊業', '03-5352-280', '108PV1123', '2019-9-16', '2019-11-20', '4.8', [True, False, True, False], '關鍵事項', '主責單位', '預計完成時間', '辦理情形'], ['勝陽能源股份有限公司', '第三型', '498.96', '屋頂型', '台北市', '台北市士林區承德路4段177號', '台北市士林區光華段四小段0793-0000地號', '地方公有屋頂', '售電', '2019-8-13', '2019-8-19', '已有併聯紀錄', '太陽能系統處/黃子嘉', '02-25984299#670', '108PV1124', '', '2019-12-9', '498.96',[False, False, True, False], '關鍵事項', '主責單位', '預計完成時間', '辦理情形']]
    #print("mylistqqqq",mylist[0])
    #for i in range(len(mylist)):
    #    for j in range(len(mylist[i])):
...
#107沒有同意備案申請日期，用核准日期代替
  ...
  #####################################################################################  
    ...
    sheet1.write_merge(46, 46, 8, 8,  mylist[28][15].replace("-","/"), style=style_other)
    sheet1.write_merge(47, 47, 8, 8,  mylist[28][16].replace("-","/"), style=style_other)
    sheet1.write_merge(48, 48, 8, 8,  mylist[28][17].replace("-","/"), style=style_other)
    sheet1.write_merge(49, 49, 8, 8,  mylist[28][18].replace("-","/"), style=style_other)
    sheet1.write_merge(50, 50, 8, 8,  mylist[28][19].replace("-","/"), style=style_other)
...
#########################################################1sdasdasdddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddssssssssssssssssssssssssssss#########################################################################here##################################################          
   ...
   # 免土地變更及免土地容許
    ...
    # 計算現在到哪階段 
    period = 0             
    _ = _get_week(int(mylist[9].split("-")[0]),int(mylist[9].split("-")[1]),int(mylist[9].split("-")[2]),today.year,today.month,today.day) 
    col_position = 9 + _ 
...
#---------------------------------------------------------------現在時間-------------------------------------------------------------------------------------
    ...
    if null:
        sheet1.write_merge(31+stage, 31+stage, col_num+default_start_week, col_num+default_start_week, "◎", style=style_gantt_color)  
    elif dd == 0:
        sheet1.write_merge(31+stage, 31+stage, col_position+2, col_position+2, "◎", style=style_gantt_color)  
    elif dd > 0:
        sheet1.write_merge(31+stage, 31+stage, col_position+2, col_position+2, "◎", style=style_delay)
    else:
        sheet1.write_merge(31+stage, 31+stage, col_position+2, col_position+2, "◎", style=style_other)  
